...
export default function RegistroPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!registroRes.ok) {
        const newErrors = {};
        if (userData.email) newErrors.email = userData.email[0];
        if (userData.password) newErrors.password = userData.password[0];
        if (userData.non_field_errors) newErrors.general = userData.non_field_errors[0];
        
        setErrors(newErrors);
        
        if (newErrors.email || newErrors.password) setCurrentStep(2);
      ...
      if (loginRes.ok) {
        ...
        setErrors({ general: 'Cuenta creada, pero falló el inicio de sesión automático.' });
      }
  ...
  return (
    <div style={{
      ...
      <header style={{
        borderRadius: '0 0 10px 10px', backgroundColor: '#ffffff', padding: '15px 20px',
        display: 'flex', alignItems: 'center', boxShadow: '0 2px 10px rgba(0, 0, 0, 0.3)'
      }}>
        <div style={{ width: '60px', height: '40px', position: 'relative' }}>
          <Image src="/Logo.png" alt="Logo" fill style={{ objectFit: 'cover', borderRadius: '50%' }} />
        </div>
      </header>

      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 20px' }}>
        <div style={{
          backgroundColor: '#f5f5f5', borderRadius: '20px', padding: '40px',
          width: '100%', maxWidth: '500px', boxShadow: '0 8px 32px rgba(0, 0, 0, 0.4)'
        }}>
            ...
            {[1, 2, 3].map(step => (
              <div key={step} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, position: 'relative', zIndex: 1 }}>
                <div style={{
                  ...
                  fontWeight: 'bold', fontSize: '14px', transition: 'all 0.3s ease', fontFamily: 'Arial, sans-serif'
                }}>
                  {step}
                </div>
          ...
          {currentStep === 1 && (
            <div style={{ animation: 'fadeIn 0.3s ease' }}>
              <div style={{ marginBottom: '20px' }}>
                ...
                <input type="text" name="nombre" value={formData.nombre} onChange={handleChange} placeholder="Tu nombre" style={inputStyle} />
                {errors?.nombre && <span style={errorTextStyle}>{errors.nombre}</span>}
              </div>
              <div style={{ marginBottom: '20px' }}>
                <label style={labelStyle}>APELLIDOS</label>
                <input type="text" name="apellidos" value={formData.apellidos} onChange={handleChange} placeholder="Tus apellidos" style={inputStyle} />
              </div>
          ...
          {currentStep === 2 && (
            <div style={{ animation: 'fadeIn 0.3s ease' }}>
              <div style={{ marginBottom: '20px' }}>
                ...
                <input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="tu@email.com" style={inputStyle} />
                {errors?.email && <span style={errorTextStyle}>{errors.email}</span>}
              </div>
              <div style={{ marginBottom: '20px' }}>
                <label style={labelStyle}>CONTRASEÑA</label>
                <input type="password" name="password" value={formData.password} onChange={handleChange} placeholder="Mínimo 8 caracteres" style={inputStyle} />
              </div>
              <div style={{ marginBottom: '20px' }}>
                <label style={labelStyle}>CONFIRMAR CONTRASEÑA</label>
                <input type="password" name="password_confirm" value={formData.password_confirm} onChange={handleChange} placeholder="Repite tu contraseña" style={inputStyle} />
            ...
            <div style={{ animation: 'fadeIn 0.3s ease' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '15px' }}>
                <div>
                  ...
                  <input type="number" name="altura" value={formData.altura} onChange={handleChange} placeholder="170" step="0.01" style={inputStyle} />
                </div>
                <div>
                  <label style={labelStyle}>PESO (kg)</label>
                  <input type="number" name="pesoActual" value={formData.pesoActual} onChange={handleChange} placeholder="70" step="0.01" style={inputStyle} />
                </div>
              </div>

              <div style={{ marginBottom: '15px' }}>
                <label style={labelStyle}>NIVEL DE ACTIVIDAD</label>
                <select name="nivel_actividad" value={formData.nivel_actividad} onChange={handleChange} style={inputStyle}>
              ...
              <div style={{ marginBottom: '20px' }}>
                ...
                <select name="objetivo" value={formData.objetivo} onChange={handleChange} style={inputStyle}>
          ...
          <p style={{ textAlign: 'center', fontSize: '13px', color: '#666', marginTop: '25px', fontFamily: 'Arial, sans-serif' }}>
            ¿Ya tienes cuenta?{' '}
            <a href="/login" style={{ color: '#1a1a1a', fontWeight: 'bold', textDecoration: 'none', borderBottom: '2px solid #1a1a1a' }}>Inicia sesión</a>
          </p>
        </div>
      </div>
...
const labelStyle = {
  display: 'block', fontSize: '12px', fontWeight: 'bold', color: '#1a1a1a', marginBottom: '8px', letterSpacing: '0.5px'
};
...
const inputStyle = {
  ...
  outline: 'none', boxSizing: 'border-box', transition: 'all 0.2s ease', cursor: 'pointer'
};

const buttonPrimaryStyle = {
  flex: 1, padding: '16px', border: 'none', borderRadius: '10px', backgroundColor: '#1a1a1a',
  color: '#fff', fontSize: '14px', textTransform: 'uppercase', letterSpacing: '1px', cursor: 'pointer', transition: 'all 0.2s ease'
};

const buttonSecondaryStyle = {
  flex: 1, padding: '16px', border: '2px solid #1a1a1a', borderRadius: '10px', backgroundColor: 'transparent',
  color: '#1a1a1a', fontSize: '14px', textTransform: 'uppercase', letterSpacing: '1px', cursor: 'pointer', transition: 'all 0.2s ease'
};

const errorTextStyle = {
  color: '#d32f2f', fontSize: '12px', marginTop: '5px', display: 'block', fontFamily: 'Arial, sans-serif'
};